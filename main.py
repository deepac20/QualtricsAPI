...
for surveyId, group in techsupport_surveyId.items():
    ...
    if group == "Intervention":
        zipfile.ZipFile(io.BytesIO(requestDownload.content)).extractall("MyQualtricsDownload/TechSupport/Intervention")
        print('Complete')
    else:
        zipfile.ZipFile(io.BytesIO(requestDownload.content)).extractall("MyQualtricsDownload/TechSupport/Controlled")
        print('Complete')

for surveyId, group in coachman_surveyId.items():
    # Setting static parameters
    requestCheckProgress = 0.0
    progressStatus = "inProgress"
    baseUrl = "https://{0}.qualtrics.com/API/v3/surveys/{1}/export-responses/".format(dataCenter, surveyId)
    ...
    # Step 4: Unzipping the file
    if group == "Intervention":
        zipfile.ZipFile(io.BytesIO(requestDownload.content)).extractall("MyQualtricsDownload/Coachman/Intervention")
        print('Complete')
    else:
        zipfile.ZipFile(io.BytesIO(requestDownload.content)).extractall("MyQualtricsDownload/Coachman/Controlled")
        print('Complete')